    ...
    if (cJSON_IsString(type) && strcmp(type->valuestring, "bno055") == 0)
    {
        ...
        if (data)
        {
            if (xSemaphoreTake(imu_data_mutex, pdMS_TO_TICKS(50)) == pdTRUE)
            {
                ...
                cJSON *quat_array = cJSON_GetObjectItem(data, "quaternion");
                if (quat_array && cJSON_GetArraySize(quat_array) >= 4)
                {
                    quat.w = cJSON_GetArrayItem(quat_array, 0)->valuedouble;
                    quat.x = cJSON_GetArrayItem(quat_array, 1)->valuedouble;
                    quat.y = cJSON_GetArrayItem(quat_array, 2)->valuedouble;
                    quat.z = cJSON_GetArrayItem(quat_array, 3)->valuedouble;
                }
